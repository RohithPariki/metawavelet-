...
# Initialize Meta-Wavelet Evaluator with N_H=2
...
def wpinn_loss(c, b):
    ...
    ic_loss = torch.mean((u_pred_ic - u_ic_left)**2)
...
def train_phase(model, optimizer, num_epochs, phase_name="Phase"):
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        c, b = model(x_collocation, t_collocation)
        total_loss, pde_loss, ic_loss, bc_loss = wpinn_loss(c, b)
        ...
        if epoch % 4000 == 0 or epoch == num_epochs - 1:
            with torch.no_grad():
                c_val, b_val = model(x_collocation, t_collocation)
                ...
                tqdm.write(f'Epoch [{epoch}/{num_epochs-1}] Loss: {total_loss.item():.4f} '
                           f'PDE: {pde_loss.item():.4f} IC: {ic_loss.item():.4f} BC: {bc_loss.item():.4f} '
...
def closure():
    ...
    c_bfgs, b_bfgs = refinement_model(x_collocation, t_collocation)
    loss, pde_loss, ic_loss, bc_loss = wpinn_loss(c_bfgs, b_bfgs)
    loss.backward()
    return loss
...
num_val = evaluator.evaluate_val(c_end, b_end).detach().cpu().numpy()
xtest_np = x_test.cpu().numpy().reshape(n_test, n_test)
ttest_np = t_test.cpu().numpy().reshape(n_test, n_test)
# exact_test is already numpy

with torch.no_grad():
    u_test_pred = evaluator.evaluate_test(c_end, b_end).cpu().numpy().reshape(n_test, n_test)

fig, axes = plt.subplots(1, 3, figsize=(18, 5))
c1 = axes[0].contourf(ttest_np, xtest_np, exact_test, 100, cmap='jet')
axes[0].set_title('Exact Solution')
fig.colorbar(c1, ax=axes[0])

c2 = axes[1].contourf(ttest_np, xtest_np, u_test_pred, 100, cmap='jet')
axes[1].set_title('MW-PINN Prediction')
fig.colorbar(c2, ax=axes[1])

c3 = axes[2].contourf(ttest_np, xtest_np, np.abs(exact_test - u_test_pred), 100, cmap='jet')
axes[2].set_title('Absolute Error')
fig.colorbar(c3, ax=axes[2])

os.makedirs('../Results_Comparison', exist_ok=True)
plt.savefig('../Results_Comparison/Heat_Conduction_sol.png', dpi=150)
print("\nPlot saved to ../Results_Comparison/Heat_Conduction_sol.png")
